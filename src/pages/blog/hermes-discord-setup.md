...
Hermes is a terminal-native AI agent that works with any LLM provider (OpenRouter, OpenAI, Anthropic, Google, local models, and 20+ others). Beyond the CLI, it has a messaging gateway that runs on 21+ platforms like Discord, Telegram, Slack, WhatsApp, Matrix, Signal, and more, giving the agent full tool access through chat, not just text responses.
...
The wizard walks you through selecting a gateway platform. I picked **Discord** and that's where the rest of this guide picks up from.
...
This is the #1 reason Discord bots fail silently. Without Message Content Intent, the bot comes online but literally cannot read what you type. If your bot is online but never responds, check this first - it's probably the issue.
...
After some investigation, the issue was that Discord treats **in-server access** and **DM access** as separate authorizations. The guild invite covers the server, but the bot application needs to be explicitly authorized on your account for DMs to work.
...
Once the application is authorized globally on your account, DMs work. This is a one-time step per account. If you ever remove and re-add the bot to a server, you may need to re-authorize it.
...
| **Messages @mentioning others but not the bot** | By default (`DISCORD_IGNORE_NO_MENTION=true`), Hermes stays silent. This prevents jumping into conversations directed at other people. |
